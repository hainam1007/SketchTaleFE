...
import { BookOpenText, FloppyDisk, Trash } from "@phosphor-icons/react";
...
export default function StoryVocabularyPage() {
  ...
  const mutation = useStoryEditorMutation(({ action, vocabularyId, payload, revision }) => action === "add" ? contentService.addVocabulary({ storyId, revision, ...payload }) : contentService.deleteVocabulary({ storyId, vocabularyId, revision }));
  ...
  function addVocabulary() {
    mutation.mutate({ action: "add", payload: values }, { onSuccess: () => setValues({ ...emptyVocabulary, pageId: story.pages[0]?.id || "" }) });
  }
  ...
  return (
    <EditorPanel eyebrow="VOCABULARY" title="Từ vựng theo ngữ cảnh" description="Mỗi từ gắn với một page để mobile có thể hiển thị đúng lúc đọc và mở rộng sang audio sau này." action={<span className="story-editor-count"><BookOpenText size={17} aria-hidden="true" /> {story.vocabulary.length} từ</span>}>
      <div className="editor-form-card">
        ...
        <button className="workspace-button" type="button" onClick={addVocabulary} disabled={mutation.isPending || !story.pages.length}><FloppyDisk size={16} aria-hidden="true" /> Thêm từ</button>
      </div>
      <EditorMutationMessage mutation={mutation} />
      {story.vocabulary.length ? <div className="editor-list" aria-label="Danh sách từ vựng">{story.vocabulary.map((item) => <article className="editor-item vocabulary-item" key={item.id}><div><span className="editor-item-index">{story.pages.find((page) => page.id === item.pageId)?.title || "Page không còn tồn tại"}</span><h3>{item.word}</h3><p>{item.meaning}</p>{item.audioUrl && <small>{item.audioUrl}</small>}</div><button className="icon-button icon-button-danger" type="button" aria-label={`Xoá từ ${item.word}`} onClick={() => mutation.mutate({ action: "delete", vocabularyId: item.id })}><Trash size={17} aria-hidden="true" /></button></article>)}</div> : <div className="editor-empty"><p>Chưa có từ vựng trong story.</p><p className="editor-help">Thêm các từ thật sự xuất hiện trong câu chuyện để phần đọc cùng bé không bị rời ngữ cảnh.</p></div>}
    </EditorPanel>
  );
}
