...
import { CheckCircle, FloppyDisk, Question, Trash } from "@phosphor-icons/react";
...
function emptyQuiz(pageId = "") {
  return { pageId, question: "", options: ["", "", ""], correctIndex: 0, feedback: "" };
}
...
export default function StoryQuizPage() {
  ...
  const mutation = useStoryEditorMutation(({ action, quizId, payload, revision }) => action === "add" ? contentService.addQuiz({ storyId, revision, ...payload }) : contentService.deleteQuiz({ storyId, quizId, revision }));
  ...
  function addQuiz() {
    mutation.mutate({ action: "add", payload: { ...values, correctIndex: Number(values.correctIndex) } }, { onSuccess: () => setValues(emptyQuiz(story.pages[0]?.id || "")) });
  }
